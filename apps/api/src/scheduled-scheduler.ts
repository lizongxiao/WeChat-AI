...
export class ScheduledScheduler {
  ...
  async testService(serviceId:string, personaId?:string) {
    ...
    for(const sub of subs){
      ...
      const result=await this.run("subscription",sub.id,sub.bot_id,sub.peer_id,sub.persona_id,prompt,Boolean(service.web_search_enabled),now,scheduledTestLockSource("subscription"),executionTime,service.timezone);
  ...
  private async run(source:"task"|"subscription",id:string,botId:string,peerId:string,personaId:string,prompt:string,web:boolean,now:Date,lockSource:string=source,executionTime:Date=now,timeZone="Asia/Shanghai"):Promise<{sent:true}|{sent:false;reason:string}>{const key=minuteKey(now);if(!(await tryAcquireScheduledExecutionLock(this.opts.db,lockSource,id,key,this.opts.lockTtlSec)))return {sent:false,reason:"duplicate_execution_lock"};try{const tok=await getContextToken(this.opts.db,botId,peerId);if(!tok)throw new Error("no_context_token");const r=await this.opts.chat.handleScheduled({botAccountId:botId,peerId,contextToken:tok,personaId,prompt,webSearchEnabled:web,source,executionTime:executionTime.toISOString(),timeZone});if(r.kind!=="reply"||!r.text)throw new Error(r.skipReason||r.kind);const out=await this.opts.sendReply(botId,peerId,r);if(!out.ok)throw new Error(out.reason||"send_failed");const patch={last_run_at:now.toISOString(),last_status:"sent",last_error:null}; source==="task"?await updateScheduledTask(this.opts.db,id,patch):await updateUserSubscription(this.opts.db,id,patch);return {sent:true};}catch(e){const reason=e instanceof Error?e.message:String(e);const patch={last_run_at:now.toISOString(),last_status:"error",last_error:reason};source==="task"?await updateScheduledTask(this.opts.db,id,patch).catch(()=>undefined):await updateUserSubscription(this.opts.db,id,patch).catch(()=>undefined);this.opts.log?.(`[schedule] ${source}=${id} failed`,e);return {sent:false,reason};}}
}
